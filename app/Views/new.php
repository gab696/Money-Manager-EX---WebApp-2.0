...
<head>
  ...
  <title><?= $edit ? 'Modifier' : 'Nouvelle transaction' ?> — MMEX Web</title>
...
<div x-data="txForm()" x-init="init()" class="mx-auto max-w-md min-h-dvh flex flex-col pt-safe">
  ...
  <header class="sticky top-0 z-20 bg-slate-50/95 backdrop-blur border-b border-slate-200">
    <div class="flex items-center justify-between h-14 px-4">
      ...
      <h1 class="text-base font-semibold">
        <span x-text="(edit ? 'Modifier ' : 'Nouvelle ') + typeMeta[type].label.toLowerCase()"></span>
      </h1>
  ...
  <section x-show="step === 1" x-transition.opacity class="flex-1 flex flex-col">
    ...
    <div class="px-3 pt-2 pb-4">
      <div class="grid grid-cols-4 gap-2">
        ...
        <button type="button" @click="press('⌫')" class="keypad-key h-14 rounded-xl bg-slate-200 text-slate-600 flex items-center justify-center" aria-label="Effacer">
  ...
  <section x-show="step === 2" x-transition.opacity class="flex-1 px-4 pt-6 space-y-2">
    ...
    <button type="button" @click="openSheet('date')"
      ...
      <span class="flex-1 text-left">
        <span class="block text-[11px] uppercase text-slate-400 tracking-wider">Date</span>
    ...
    <button type="button" @click="openSheet('account')"
      ...
      <span class="h-9 w-9 rounded-lg bg-slate-100 flex items-center justify-center text-lg">🏦</span>
      <span class="flex-1 text-left">
        <span class="block text-[11px] uppercase text-slate-400 tracking-wider" x-text="type === 'Transfer' ? 'Depuis' : 'Compte'"></span>
        <span class="block text-sm font-medium" :class="account ? '' : 'text-slate-400'" x-text="account || 'Choisir…'"></span>
      </span>
      <svg class="h-5 w-5 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
    </button>
    ...
    <template x-if="type === 'Transfer'">
      <button type="button" @click="openSheet('toAccount')"
        ...
        <span class="h-9 w-9 rounded-lg bg-slate-100 flex items-center justify-center text-lg">🏦</span>
        <span class="flex-1 text-left">
          <span class="block text-[11px] uppercase text-slate-400 tracking-wider">Vers</span>
          <span class="block text-sm font-medium" :class="toAccount ? '' : 'text-slate-400'" x-text="toAccount || 'Choisir…'"></span>
        </span>
        <svg class="h-5 w-5 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
      </button>
    </template>
  ...
  <section x-show="step === 3 && stepCount() === 4" x-transition.opacity class="flex-1 px-4 pt-6 space-y-2">
    ...
    <button type="button" @click="openSheet('payee')"
      ...
      <span class="h-9 w-9 rounded-lg bg-slate-100 flex items-center justify-center text-lg">🧾</span>
      <span class="flex-1 text-left">
        <span class="block text-[11px] uppercase text-slate-400 tracking-wider">Bénéficiaire / Tiers</span>
        <span class="block text-sm font-medium" :class="payee ? '' : 'text-slate-400'" x-text="payee?.PayeeName ?? 'Facultatif'"></span>
      </span>
      <template x-if="payee"><button @click.stop="clearPayee()" class="p-1 text-slate-400 hover:text-rose-500" aria-label="Effacer">✕</button></template>
      <svg class="h-5 w-5 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
    </button>
    <?php endif; ?>
    ...
    <button type="button" @click="openSheet('category')"
      ...
      <span class="flex-1 text-left">
        <span class="block text-[11px] uppercase text-slate-400 tracking-wider">Catégorie</span>
        <span class="block text-sm font-medium" :class="category ? '' : 'text-slate-400'" x-text="category ? categoryLabel(category) : 'Choisir…'"></span>
      </span>
      <template x-if="category"><button @click.stop="clearCategory()" class="p-1 text-slate-400 hover:text-rose-500" aria-label="Effacer">✕</button></template>
      <svg class="h-5 w-5 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
    </button>
    <?php endif; ?>
  </section>
  ...
  <section x-show="step === stepCount()" x-transition.opacity class="flex-1 px-4 pt-6 pb-4 space-y-4">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      <div class="px-4 py-4 text-center border-b border-slate-100">
        <div class="text-xs uppercase tracking-wider text-slate-400 mb-1">Montant</div>
      ...
      <dl class="divide-y divide-slate-100">
        ...
        <div class="px-4 py-2.5 flex items-center justify-between text-sm">
          <dt class="text-slate-500" x-text="type === 'Transfer' ? 'Depuis' : 'Compte'"></dt>
          <dd class="font-medium truncate ml-2" x-text="account || '—'"></dd>
        </div>
        <template x-if="type === 'Transfer'">
          <div class="px-4 py-2.5 flex items-center justify-between text-sm">
            <dt class="text-slate-500">Vers</dt>
            <dd class="font-medium truncate ml-2" x-text="toAccount || '—'"></dd>
          </div>
        </template>
        <template x-if="type !== 'Transfer'">
          <div class="px-4 py-2.5 flex items-center justify-between text-sm">
            <dt class="text-slate-500">Bénéficiaire / Tiers</dt>
    ...
    <!-- Notes -->
    <label class="block">
      <span class="block text-[11px] uppercase text-slate-400 tracking-wider mb-1.5">Notes (facultatif)</span>
      <textarea x-model="notes" rows="3" placeholder="Précisions, n° de ticket…"
                class="w-full p-3 rounded-xl border border-slate-200 text-base bg-white resize-none"></textarea>
    </label>
  </section>

  <!-- ================= Footer : navigation step ================= -->
  <footer class="sticky bottom-0 z-20 bg-white/95 backdrop-blur border-t border-slate-200 pb-safe">
    <div class="mx-auto max-w-md px-4 py-3 grid grid-cols-[auto_1fr] gap-2">
      <button type="button" @click="goBack()"
        ...
        <span x-text="step === 1 ? 'Annuler' : 'Précédent'"></span>
      </button>
      <button type="button" @click="step < stepCount() ? goNext() : save()"
        ...
        <span x-show="step < stepCount()" x-text="'Suivant →'"></span>
        <span x-show="step === stepCount()" x-text="saving ? 'Enregistrement…' : (edit ? 'Mettre à jour' : '✓ Enregistrer')"></span>
      </button>
    </div>
  </footer>
  ...
  <section x-show="sheet === 'date'" x-transition.duration.200ms
    ...
    <div class="mx-auto max-w-md p-5">
      ...
      <div class="mt-3 flex gap-2">
        <button @click="setDate(0)"  class="flex-1 h-10 rounded-lg bg-slate-100 text-sm">Aujourd'hui</button>
        <button @click="setDate(-1)" class="flex-1 h-10 rounded-lg bg-slate-100 text-sm">Hier</button>
        <button @click="setDate(-2)" class="flex-1 h-10 rounded-lg bg-slate-100 text-sm">Avant-hier</button>
      </div>
      <button @click="closeSheet()" class="mt-5 w-full h-12 rounded-xl bg-indigo-600 text-white font-semibold">Valider</button>
    </div>
  </section>
    ...
    <section x-show="sheet === field" x-transition.duration.200ms
      ...
      <div class="mx-auto w-full max-w-md p-5 pb-2">
        ...
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-lg font-semibold" x-text="field === 'account' ? (type === 'Transfer' ? 'Compte source' : 'Compte') : 'Compte destination'"></h3>
          <button @click="closeSheet()" class="p-2 text-slate-500">✕</button>
        </div>
        <input type="text" x-model="search" placeholder="Rechercher ou créer un compte…"
      ...
      <div class="mx-auto w-full max-w-md flex-1 overflow-y-auto px-5 pb-5 no-scrollbar">
        <template x-if="search.length === 0 && frequent.accounts.length > 0">
          <div>
            <div class="text-[11px] uppercase tracking-wider text-slate-400 mt-2 mb-1">Fréquents</div>
        ...
        <div class="text-[11px] uppercase tracking-wider text-slate-400 mt-3 mb-1">Tous</div>
  ...
  <section x-show="sheet === 'category'" x-transition.duration.200ms
    ...
    <div class="mx-auto w-full max-w-md p-5 pb-2">
      ...
      <input type="text" x-model="search" placeholder="Rechercher ou créer…"
             class="w-full h-11 px-4 rounded-xl border border-slate-200 text-base bg-slate-50">
    </div>
    <div class="mx-auto w-full max-w-md flex-1 overflow-y-auto px-5 pb-5 no-scrollbar">
      ...
      <template x-if="category && search.length === 0">
        <button @click="clearCategory(); closeSheet()" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl bg-rose-50 text-rose-700 text-left">
          ...
          <span class="flex-1 text-sm font-medium">Aucune catégorie</span>
        </button>
      </template>

      <template x-if="search.length === 0 && frequent.categories.length > 0">
        <div>
          <div class="text-[11px] uppercase tracking-wider text-slate-400 mt-3 mb-1">Fréquentes</div>
      ...
      <div class="text-[11px] uppercase tracking-wider text-slate-400 mt-3 mb-1">Toutes</div>
      ...
      <div class="mt-4 pt-4 border-t border-slate-100">
        <div class="text-[11px] uppercase tracking-wider text-slate-400 mb-2">Nouvelle catégorie</div>
        <div class="space-y-2">
          <input type="text" x-model="newCat.category" placeholder="Catégorie parent (ex. Alimentation)"
                 class="w-full h-11 px-3 rounded-xl border border-slate-200 text-sm">
          <input type="text" x-model="newCat.subcategory" placeholder="Sous-catégorie (facultatif, ex. Courses)"
                 class="w-full h-11 px-3 rounded-xl border border-slate-200 text-sm">
  ...
  <section x-show="sheet === 'payee'" x-transition.duration.200ms
    ...
    <div class="mx-auto w-full max-w-md p-5 pb-2">
      <div class="h-1 w-10 bg-slate-200 rounded mx-auto mb-4"></div>
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-lg font-semibold">Bénéficiaire / Tiers</h3>
        <button @click="closeSheet()" class="p-2 text-slate-500">✕</button>
      </div>
      <input type="text" x-model="search" placeholder="Rechercher ou créer…"
             class="w-full h-11 px-4 rounded-xl border border-slate-200 text-base bg-slate-50">
    ...
    <div class="mx-auto w-full max-w-md flex-1 overflow-y-auto px-5 pb-5 no-scrollbar">
      <template x-if="payee && search.length === 0">
        <button @click="clearPayee(); closeSheet()" class="w-full flex items-center gap-3 px-3 py-3 rounded-xl bg-rose-50 text-rose-700 text-left">
          <span class="h-8 w-8 rounded-full bg-rose-100 flex items-center justify-center text-xs">✕</span>
          <span class="flex-1 text-sm font-medium">Aucun bénéficiaire</span>
        </button>
      </template>

      <template x-if="search.length === 0 && frequent.payees.length > 0">
        <div>
          <div class="text-[11px] uppercase tracking-wider text-slate-400 mt-3 mb-1">Fréquents</div>
      ...
      <div class="text-[11px] uppercase tracking-wider text-slate-400 mt-3 mb-1">Tous (A→Z)</div>
