<?php /** @var callable $e */ /** @var string $baseUrl */ /** @var string $csrf */ ?>
<!doctype html>
<html lang="fr">
<head>
  <meta charset="utf-8">
  ...
  <title>Premier lancement — MMEX Web</title>
...
<main class="mx-auto max-w-md min-h-dvh flex flex-col px-6 pt-safe pb-safe">
  <div class="flex-1 flex flex-col justify-end pb-8">
    <div class="text-white">
      ...
      <h1 class="text-2xl font-semibold">Premier lancement</h1>
      <p class="mt-2 text-indigo-100 text-sm">Choisis un identifiant et un mot de passe. La base SQLite a été créée automatiquement.</p>
    </div>
  </div>
  ...
  <form method="post" action="<?= $e($baseUrl) ?>/setup" class="bg-white rounded-3xl shadow-xl p-6 space-y-4">
    ...
    <?php if (!empty($_GET['error'])): ?>
      <div class="px-3 py-2 rounded-lg bg-rose-50 text-rose-700 text-sm">Identifiant requis, mot de passe ≥ 4 caractères.</div>
    <?php endif; ?>
    <div>
    ...
    <button type="submit" class="w-full h-12 rounded-xl bg-indigo-600 text-white font-semibold">Créer le compte</button>
  </form>

  <p class="mt-6 text-center text-xs text-indigo-100/80">
    Un GUID de sync a été généré. Tu le retrouveras dans Réglages pour le copier dans MMEX desktop.
  </p>
</main>
